...
import { PlatonusHttpError, platonusFetch, platonusFetchBinary } from "@/lib/platonus/server";
import { normalizeBaseUrl } from "@/lib/platonus/endpoints";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      const info = await platonusFetch(`rest/mobile/personInfo/${ctx.lang}`, {
        baseUrl: ctx.baseUrl,
        token: ctx.token,
      });
    ...
    for (const path of picturePaths) {
      try {
        const binary = await platonusFetchBinary(path, {
          baseUrl: ctx.baseUrl,
          token: ctx.token,
        });
        ...
        // sometimes Platonus returns base64 as text/plain
        if (
        ...
        if (err instanceof PlatonusHttpError && (err.status === 401 || err.status === 403)) {
          throw err;
        }
      }
    }
